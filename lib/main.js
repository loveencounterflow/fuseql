(function() {
  var CND, FUSE, SQLITE3, alert, badge, db, db_route, debug, echo, fallback_gid, fallback_mode, fallback_size, fallback_time, fallback_uid, help, info, is_valid_route, later, log, mount_locator, mount_route, njs_path, result_codes, rpr, split_route, sqlitefs, urge, warn, whisper,
    slice = [].slice;
      ...
      switch (false) {
        ...
        default:
          results.push((function(name) {
            ...
            return sqlitefs[name] = function() {
              var _, handler, j;
              _ = 2 <= arguments.length ? slice.call(arguments, 0, j = arguments.length - 1) : (j = 0, []), handler = arguments[j++];
              warn(message);
              return handler(result_codes['error'], message);
            };
          })(name));
      }
    }
